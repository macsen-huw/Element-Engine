...
----------------------------------------------------------------------
  ...
  local function addconfigs(...)
    ...
    -- update with https://premake.github.io/docs/configurations/
	  local newconfigs = { }
  ...
  --We are forcing double or single
  --[[
...
----------------------------------------------------------------------
  ...
  project "ode"
    ...
    -- Taken from workspace setup
    --gotta prevent E0167	argument of type "char *" is incompatible with parameter of type "LPCWSTR"
    --TODO, does this work on *nix?
    characterset("MBCS")
    includedirs {
      "../include",
      "../ode/src"
    ...
    -- We are forcing staticlib
    --[[
    filter { "only-static or *Lib" }
      kind    "StaticLib"
      defines "ODE_LIB"
      
    --configuration { "only-shared or *DLL" }
    filter { "only-shared or *DLL" }
      kind    "SharedLib"
      defines "ODE_DLL"
    ]
...
----------------------------
  if _ACTION and _ACTION ~= "clean" then
    ...
    --TODO change this if you want SINGLE/DOUBLE
    --Force double
    generate("SINGLE")
